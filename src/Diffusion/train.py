    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, (input_image, noise_levels, target_image) in enumerate(train_loader):
            ...
            if (batch_idx % save_interval == 0):
                ...
                reconstructed_images = [reconstruct_image_iteratively(model, initial_noisy_image, 150).cpu().detach().numpy().squeeze(0) for initial_noisy_image in initial_noisy_images] 

                save_reconstructed_images(epoch, batch_idx, reconstructed_images)#, lrate=lr)
...
if __name__ == '__main__':
    ...
    train_dataset = DiffusionDataset("../data/diffused_train/", transform=transforms.ToTensor()) # num_images=25,
